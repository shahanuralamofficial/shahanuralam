  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="min-h-screen bg-[#080605] text-slate-200 font-sans pb-20">
      <nav className="max-w-7xl mx-auto px-4 sm:px-6 py-6 flex justify-between items-center sticky top-0 bg-[#080605]/80 backdrop-blur-xl z-50 border-b border-white/5">
        <button onClick={onBack} className="flex items-center gap-2 text-slate-400 hover:text-orange-500 transition-all bg-white/5 px-4 py-2 rounded-xl border border-white/5">
          <ArrowLeft size={18} /> <span className="font-bold">Portfolio</span>
        </button>
        <span className="font-black uppercase tracking-widest text-sm text-orange-500 flex items-center gap-2">
      ...
      <div className="max-w-6xl mx-auto px-4 mt-12">
        <header className="mb-12">
          <h1 className="text-5xl font-black text-white tracking-tighter mb-4">Master the <span className="text-orange-500">Algorithms</span></h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
           ...
           <div className="lg:col-span-1 space-y-4">
                 ...
                 <div className="space-y-2">
                    {[
                        ...
                        <button
                            ...
                            disabled={isSorting}
                            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl text-xs font-bold transition-all border ${visType === t.id ? 'bg-orange-500 text-black border-orange-500 shadow-lg shadow-orange-500/20' : 'text-slate-400 border-transparent hover:border-white/10 hover:bg-white/5'}`}
                        >
              ...
              <div className="p-6 rounded-[2rem] bg-orange-500/5 border border-orange-500/10">
                 <h4 className="text-[10px] font-black uppercase tracking-widest text-orange-500 mb-4 flex items-center gap-2"><Info size={12}/> Info</h4>
                 ...
                 <div className="flex justify-between items-center mb-12">
                    ...
                    <div className="flex gap-3">
                       ...
                       <button
                         ...
                         disabled={isSorting}
                         className="flex items-center gap-2 px-8 py-3 bg-orange-500 text-black font-black text-xs uppercase tracking-widest rounded-xl hover:bg-orange-400 transition-all"
                       >
                        ...
                        <div className="grid grid-cols-5 sm:grid-cols-10 gap-2 w-full max-w-2xl">
                            {visArray.map((val, i) => (
                                ...
                                <motion.div
                                    ...
                                    className={`aspect-square flex items-center justify-center rounded-xl text-[10px] font-black border transition-colors ${visPointers.active.includes(i) ? 'bg-orange-500 border-orange-500 text-black shadow-lg shadow-orange-500/30' : 'bg-white/5 border-white/5 text-slate-400'}`}
                                >
                                    {i}
                                </motion.div>
                                )
